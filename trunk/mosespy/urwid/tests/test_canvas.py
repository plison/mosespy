...
from mosespy.urwid import canvas
from mosespy.urwid.compat import B
...
class CanvasCacheTest(unittest.TestCase):
    def setUp(self):
        # purge the cache
        mosespy.urwid.CanvasCache._widgets.clear()

    def cct(self, widget, size, focus, expected):
        got = mosespy.urwid.CanvasCache.fetch(widget, mosespy.urwid.Widget, size, focus)
        assert expected==got, "got: %s expected: %s"%(got, expected)

    def test1(self):
        a = mosespy.urwid.Text("")
        b = mosespy.urwid.Text("")
        blah = mosespy.urwid.TextCanvas()
        blah.finalize(a, (10,1), False)
        blah2 = mosespy.urwid.TextCanvas()
        blah2.finalize(a, (15,1), False)
        bloo = mosespy.urwid.TextCanvas()
        bloo.finalize(b, (20,2), True)

        mosespy.urwid.CanvasCache.store(mosespy.urwid.Widget, blah)
        mosespy.urwid.CanvasCache.store(mosespy.urwid.Widget, blah2)
        mosespy.urwid.CanvasCache.store(mosespy.urwid.Widget, bloo)
        ...
        mosespy.urwid.CanvasCache.invalidate(a)
...
class CanvasTest(unittest.TestCase):
    def ct(self, text, attr, exp_content):
        c = mosespy.urwid.TextCanvas([B(t) for t in text], attr)
...
class CanvasJoinTest(unittest.TestCase):
    def cjtest(self, desc, l, expected):
        ...
        result = list(mosespy.urwid.CanvasJoin(l).content())

        assert result == expected, "%s expected %r, got %r"%(
            desc, expected, result)

    def test(self):
        C = mosespy.urwid.TextCanvas
        hello = C([B("hello")])
        there = C([B("there")], [[("a",5)]])
        a = C([B("a")])
        hi = C([B("hi")])
...
class CanvasOverlayTest(unittest.TestCase):
    def cotest(self, desc, bgt, bga, fgt, fga, l, r, et):
        bgt = B(bgt)
        fgt = B(fgt)
        bg = mosespy.urwid.CompositeCanvas(
            mosespy.urwid.TextCanvas([bgt],[bga]))
        fg = mosespy.urwid.CompositeCanvas(
            mosespy.urwid.TextCanvas([fgt],[fga]))
        bg.overlay(fg, l, 0)
    ...
    def test3(self):
        mosespy.urwid.set_encoding("euc-jp")
...
class CanvasPadTrimTest(unittest.TestCase):
    def cptest(self, desc, ct, ca, l, r, et):
        ct = B(ct)
        c = mosespy.urwid.CompositeCanvas(
            mosespy.urwid.TextCanvas([ct], [ca]))
